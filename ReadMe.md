...
A: If in multi-experiment mode, make sure you have saved your signals dataset. If this is not the case, make sure you wait for sufficient time for this analysis to take place. If this problem persists, then it might be due to a problem with the python environment not being configured correctly on the computer. Otherwise, it may be due to the appropriate packages for NeuroMaps were not installed. Please review installation guidelines. To troubleshoot, running NeuroMaps.m would help rather than NeuroMaps.exe to output relevant errors that would outline the source of the issue.
