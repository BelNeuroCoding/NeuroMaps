function study_evolution(h)
    ...
    if isfield(h,'group_evolution') && isvalid(h.group_evolution)
        delete(setdiff(allchild(h.group_evolution),h.group_evolution_button));
    end
    ...
    for m = 1:nMetrics
        ...
        for g = 1:nGroups
            ...
            switch metric_name
                ...
                case {'Impedance','Capacitance','Aperiodic Offset','Aperiodic Exponent'}
                    ...
                    if useDenseOrCombo(numel(combo_ids))
                        if length(mask_spec)~=length(vec)
                                vals = [];
                            else
                                vals = vec(mask_spec);
                        end
        ...
        plot_box_per_group(ax, group_data, group_labels,colors, metric_name);        
                ...
                switch statMethod
                    ...
                    case 'Pairwise t-tests'
                        ...
                        for k = 1:size(combs,1)
                            ...
                            if ~isempty(star)
                                plot(ax,[g1 g2],y_offsets(k)*[1 1],'k-','LineWidth',1.5);
                                text(mean([g1 g2]),y_offsets(k)+0.1*yspan,star,'HorizontalAlignment','center','Color','k');
                            end
                        end
                        if any(y_offsets), ylim(ax,[0 max(y_offsets)*1.05]); end
                end
...
% Create dialog
d = dialog( ...
    'Name','Please Enter Corresponding Experiment Ages', ...
    'Units','pixels', ...
...
p = uipanel( ...
    ...
    'Units','pixels', ...
    'Position',[20 335 440 115], ...
    'BackgroundColor',[1 1 1]);

helpText = sprintf([ ...
    'Enter the biological/sample age for each selected Exp/Port at a known reference date.\n\n' ...
    'Age (days): sample age on the Start Date, e.g. 30 for DIV30.\n' ...
    'Start Date: the calendar date corresponding to that age, using yyyy-MM-dd.\n\n' ...
    'Example: if Exp1_Port1 was DIV30 on 2025-01-01, enter Age=30 and Start Date=2025-01-01.' ...
    ]);
...
function plot_box_per_group(ax, group_data,group_labels, colors, ylab)
...
for k = 1:nGroups
    ...
    scatter(ax, x, data_mat(valid_points,k), 10, 'k', 'filled', 'MarkerFaceAlpha', 0.5,'DisplayName',group_labels{k});
end
