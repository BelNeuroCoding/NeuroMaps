...
function results_out = fooof_unpack_results(results_in)
    results_out = struct();
    py_array = py.importlib.import_module('array');

    results_out.aperiodic_params = ...
        double(py_array.array('d', results_in.aperiodic_params));

    temp = double(py_array.array('d', results_in.peak_params.ravel));
    results_out.peak_params = ...
        transpose(reshape(temp, 3, length(temp) / 3));

    temp = double(py_array.array('d', results_in.gaussian_params.ravel));
    results_out.gaussian_params = ...
        transpose(reshape(temp, 3, length(temp) / 3));

    results_out.error = ...
        double(py_array.array('d', py.numpy.nditer(results_in.error)));
