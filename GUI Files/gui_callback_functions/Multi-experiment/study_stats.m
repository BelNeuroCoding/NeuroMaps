        ...
        for g = 1:nGroups
            ...
            switch metric_name
                ...
                case 'Impedance'
                    ...
                    continue;
                case 'Capacitance'
                    capacitance = cs.capacitance(mask_spec); data_for_plot{g} = capacitance;
                    continue;
                case 'Aperiodic Offset'
                    if length(mask_spec)~=length(cs.offset)
                        data_for_plot{g}=[];
                    else
                        aperiodic_off = cs.offset(mask_spec); data_for_plot{g} = aperiodic_off;
                    end
                    continue;
                case 'Aperiodic Exponent'
                    if length(mask_spec)~=length(cs.exponent)
                        data_for_plot{g}=[];
                    else
                        aperiodic_exp = cs.exponent(mask_spec);  data_for_plot{g} = aperiodic_exp;
                    end
                    continue;
                ...
                switch statMethod
                    ...
                    case 'Pairwise t-tests'
                        ...
                        for k = 1:size(combs,1)
                            ...
                            if p_pair<0.001, star='***'; elseif p_pair<0.01, star='**'; elseif p_pair<0.05, star='*'; else star='n.s.'; end
                            y_offsets(k) = ylims(2)+0.08*yspan+0.05*k*yspan;
                            if ~isempty(star)
                                plot(ax,[g1 g2],y_offsets(k)*[1 1],'k-','LineWidth',1.5);
                                text(mean([g1 g2]),y_offsets(k)+0.1*yspan,star,'HorizontalAlignment','center','Color','k');
                            end
