...
for i = 1:size(selected,1)
    ...
    if isfield(h,'osc_table_fig') && isvalid(h.osc_table_fig)
        ...
        h.osc_table_fig = uifigure('Name','Oscillatory Power Table');

        % Create grid layout: table (top) + button (bottom)
        gl = uigridlayout(h.osc_table_fig, [2 1]);
        ...
        % Table in row 1
        h.osc_table = uitable(gl, ...
            'Data', tableData, ...
            'ColumnName', tableData.Properties.VariableNames);
        
        % Button in row 2
        if ~isfield(h,'saveTableBtn') || ~isvalid(h.saveTableBtn)
        h.saveTableBtn = uibutton(gl, ...
            'Text','Save Table', ...
            'ButtonPushedFcn', @(src,event) saveoscTable(h.osc_table.Data));
        end
% 
% h.osc_table_fig = uifigure('Name','Oscillatory Power Table');
%         tableData = table(chans', total_osc_power, ...
...
%                              'Units','normalized','Position',[0 0 1 1]);
    ...
    % if ~isfield(h,'saveTableBtn') || ~isvalid(h.saveTableBtn)
    %     h.saveTableBtn = uibutton(h.osc_table_fig, ...
    %         'Text','Save Table', ...
    %         'Position',[320 10 80 30], ...
    %         'ButtonPushedFcn', @(src,event) saveoscTable(h.osc_table.Data));
    % end
end
