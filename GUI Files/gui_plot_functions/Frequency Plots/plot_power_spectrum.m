function plot_power_spectrum(h)
    ...
    if size(selected,1) > 1 || h.pspec_toggle.Value
        ...
        delete(axesToDelete);
        h.ps_axes = subplot(1,1,1, 'Parent', h.pspec_tab);
        axes(h.ps_axes)
        all_psdsm = {};
        all_freqs = {};
        port_labels ={};
        ...
        for selIdx = 1:size(selected,1)
            ...
            loglog(f_axis, mean(medianPSD,2)', 'Color',colors(selIdx,:), 'LineWidth', 1.5);  % Median PSD
        ...
        loglog(f_axis, medianPSD, 'b-', 'LineWidth', 1.5);  % Median PSD
        hold on;
        loglog(f_axis, PSDw, 'r-.', 'LineWidth', 1.5); % Welch PSD
        hold off;
